...
const uint8_t LCD_T::font[256][5]  =
    {
...
void LCD_T::init(void)
    ...
    write_cmd_byte(ILI9488_CMD_NEGATIVE_GAMMA_CTRL); //N-Gamma
    write_data_byte(0x00);
    ...
    write_cmd_byte(ILI9488_CMD_POWER_CONTROL_1); //Power Control 1
    write_data_byte(0x17); //Vreg1out
    write_data_byte(0x15); //Verg2out

    write_cmd_byte(ILI9488_CMD_POWER_CONTROL_2); //Power Control 2
    write_data_byte(0x41); //VGH,VGL

    write_cmd_byte(ILI9488_CMD_POWER_CONTROL_3); //Power Control 3
    write_data_byte(0x00);
    write_data_byte(0x12); //Vcom
    write_data_byte(0x80);

    write_cmd_byte(ILI9488_CMD_MEMORY_ACCESS_CONTROL); //Memory Access
    write_data_byte(0x40);

    write_cmd_byte(ILI9488_CMD_INTERFACE_PIXEL_FORMAT); // Interface Pixel Format
    write_data_byte(0x66);

    write_cmd_byte(ILI9488_CMD_INTERFACE_MODE_CONTROL); // Interface Mode Control
    write_data_byte(0x00);

    write_cmd_byte(ILI9488_CMD_FRAME_RATE_CONTROL_NORMAL); //Frame rate
    write_data_byte(0xA0); //60Hz

    write_cmd_byte(ILI9488_CMD_DISPLAY_INVERSION_CONTROL); //Display Inversion Control
    write_data_byte(0x02); //2-dot

    write_cmd_byte(ILI9488_CMD_DISPLAY_FUNCTION_CONTROL); //RGB/MCU Interface Control
    write_data_byte(0x02); //MCU
    write_data_byte(0x01); //Source,Gate scan dieection
    //write_data_byte(0x00);


    write_cmd_byte(ILI9488_CMD_SET_IMAGE_FUNCTION); // Set Image Function
    write_data_byte(0x00); // Disable 24 bit data input

    write_cmd_byte(ILI9488_CMD_ADJUST_CONTROL_3); // Adjust Control 3
    ...
    write_cmd_byte(ILI9488_CMD_SLEEP_OUT); //Sleep out
    delay(100000);
    write_cmd_byte(ILI9488_CMD_DISPLAY_ON); //Display on
    delay(500000);
    }

void LCD_T::set_stdout_params(uint16_t x, uint16_t y, uint32_t bg_color, uint32_t text_color)
    {
    }


void LCD_T::set_area(uint16_t start_x,uint16_t end_x, uint16_t start_y, uint16_t end_y)
    {
    write_cmd_byte(ILI9488_CMD_COLUMN_ADDRESS_SET);
    write_data_byte((start_x>>8)&0xff);
    write_data_byte(start_x&0xff);
    write_data_byte((end_x>>8)&0xff);
    write_data_byte((end_x)&0xff);

    write_cmd_byte(ILI9488_CMD_PAGE_ADDRESS_SET);
    write_data_byte((start_y>>8)&0xff);
    write_data_byte(start_y&0xff);
    write_data_byte((end_y>>8)&0xff);
    write_data_byte((end_y)&0xff);
    }


void LCD_T::clear_screen(void)
    {
    set_area(0,WIDTH-1,0,HEIGHT-1);
    write_cmd_byte(ILI9488_CMD_MEMORY_WRITE);
    LCD_IO_DATA_WRITE = 0xff;
    for(uint32_t i=0; i<=(WIDTH)*(HEIGHT); i++)
        {
        LCD_IO_CTRL_WRITE &= ~((1<<IO_CTRL_WR_bit));
        LCD_IO_CTRL_WRITE |= (1<<IO_CTRL_WR_bit);
        LCD_IO_CTRL_WRITE &= ~((1<<IO_CTRL_WR_bit));
        LCD_IO_CTRL_WRITE |= (1<<IO_CTRL_WR_bit);
        LCD_IO_CTRL_WRITE &= ~((1<<IO_CTRL_WR_bit));
...
void LCD_T::show_rect(uint16_t start_x,uint16_t end_x,uint16_t start_y, uint16_t end_y,uint32_t color)
    {
    set_area(start_x,end_x,start_y,end_y);
    write_cmd_byte(ILI9488_CMD_MEMORY_WRITE);
    uint16_t width = (end_y-start_y);
    uint16_t heigth = (end_x-start_x);
    for(uint32_t i=0; i!=(width*heigth); i++)
        {
        write_data_byte(color);
        write_data_byte(color>>8);
        write_data_byte(color>>16);
        }
    }
		


inline int16_t abs(int16_t in)
		{
			return (in<0)?-in:in;
		}
		
void LCD_T::line(uint16_t start_x,uint16_t end_x,uint16_t start_y, uint16_t end_y,uint32_t color, uint8_t line_width)
    {
		int16_t width = end_y-start_y;
    int16_t heigth = end_x-start_x;
		if(!width)
				return;
		if(!heigth)
				return;
		...
		float next = 0;
		uint16_t y = start_y;
		float coef = (start_x> end_x) ? -((float)width/(float)heigth):((float)width/(float)heigth);
	
		for( uint16_t x = start_x; x!=end_x; x = (start_x > end_x)?x-1:x+1 )
			{
			set_area(x,x+line_width,y,y+line_width);
			next = next+coef;
			y = next+start_y;
			write_cmd_byte(ILI9488_CMD_MEMORY_WRITE);
			for(uint8_t lw = 0; lw < (line_width*line_width); lw++)
				{
...
void LCD_T::show_pixel(uint32_t color)
    {
    write_data_byte(color&0xff); //R
    write_data_byte((color>>8)&0xff); //R
    write_data_byte((color>>16)&0xff); //R
    }

void LCD_T::show_mem(uint16_t x,uint16_t y, uint8_t width, uint8_t heigth, uint8_t* mem)
    {
    set_area(x,x+width-1,y,y+heigth-1);

    write_cmd_byte(ILI9488_CMD_MEMORY_WRITE);
    for(uint32_t i=0; i!=(width*heigth)*3; i+=3)
        {
        show_pixel(mem[i]|mem[i+1]<<8|mem[i+2]<<16);
        }
    }


void	LCD_T::show_rle_mem(uint16_t x,uint16_t y, uint8_t width, uint8_t heigth, uint8_t *img)
    {
    set_area(x,x+width-1,y,y+heigth-1);
    write_cmd_byte(ILI9488_CMD_MEMORY_WRITE);
    uint32_t out_pointer = 0;
    uint32_t in_pointer = 0;
    while (out_pointer<(width*heigth*3))
        {
        unsigned int p_data = img[in_pointer++];
        if (p_data & 128)
            {
            p_data = p_data - 128;
            do
                {
                show_pixel(img[in_pointer+2]|img[in_pointer+1]<<8|img[in_pointer]<<16);
                out_pointer+=3;
                }
            while(--p_data);
            in_pointer+=3;
...
void LCD_T::putchar(uint16_t x, uint16_t y, uint8_t chr,uint32_t bg_color,uint32_t text_color)
    {
    set_area(x,x+4,y,y+7);
    write_cmd_byte(ILI9488_CMD_MEMORY_WRITE);
...
void LCD_T::write_cmd_byte(uint8_t cmd)
    {
    LCD_IO_DATA_DIR = IO_DATA_DIR_WRITE_MASK;
    LCD_IO_DATA_WRITE = reverse(cmd);
			
    LCD_IO_CTRL_WRITE &= ~((1<<IO_CTRL_RS_bit)|(1<<IO_CTRL_WR_bit));
			
    LCD_IO_CTRL_WRITE |= (1<<IO_CTRL_WR_bit)|(1<<IO_CTRL_RS_bit);
			
    }

void LCD_T::write_data_byte(uint8_t data)
    {
    LCD_IO_DATA_DIR = IO_DATA_DIR_WRITE_MASK;
    LCD_IO_DATA_WRITE = reverse(data);
...
uint8_t LCD_T::read_data_byte(void)
    {
    uint8_t data = 0;
    LCD_IO_DATA_DIR = IO_DATA_DIR_READ_MASK;
    LCD_IO_CTRL_WRITE &= ~((1<<IO_CTRL_RD_bit));
    data = reverse((uint8_t)LCD_IO_DATA_READ);
    LCD_IO_CTRL_WRITE |= (1<<IO_CTRL_RD_bit);
    return data;
    }
...
void LCD_T::io_init(void)
    {
		...
		GPIO_InitStructure.GPIO_Pin   = (1<<IO_CTRL_CS_bit)|(1<<IO_CTRL_RS_bit)|(1<<IO_CTRL_RST_bit)|(1<<IO_CTRL_RD_bit)|(1<<IO_CTRL_WR_bit);
		GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_Out_PP;
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_Init(LCD_IO_CTRL , &GPIO_InitStructure);
			
    LCD_IO_DATA_DIR = IO_DATA_DIR_WRITE_MASK;
			
    LCD_IO_CTRL_WRITE |= (1<<IO_CTRL_RST_bit)|(1<<IO_CTRL_CS_bit)|(1<<IO_CTRL_WR_bit)|(1<<IO_CTRL_RD_bit)|(1<<IO_CTRL_RS_bit);
    delay(10);
    LCD_IO_CTRL_WRITE &= ~(1<<IO_CTRL_RST_bit);
    delay(10);
    LCD_IO_CTRL_WRITE |= (1<<IO_CTRL_RST_bit);
    delay(250);
    }

uint8_t LCD_T::reverse(uint8_t b)
    {
   // b = (b & 0xF0) >> 4 | (b & 0x0F) << 4;
   // b = (b & 0xCC) >> 2 | (b & 0x33) << 2;
   // b = (b & 0xAA) >> 1 | (b & 0x55) << 1;
...
void LCD_T::cs_set(void)
    {
    LCD_IO_CTRL_WRITE |= (1<<IO_CTRL_CS_bit);
    delay(5000);
    }

void LCD_T::cs_reset(void)
    {
    LCD_IO_CTRL_WRITE &= ~(1<<IO_CTRL_CS_bit);
    delay(50);
    }

void LCD_T::delay(volatile uint32_t d)
    {
    while(--d);
    }
