/* aes.c */

#include <stdint.h>
#include <string.h>
#include "aes.hpp"

/**< ��: https://github.com/DavyLandman/AESLib */

const uint8_t sbox[256] =
	{
	
	};

const uint8_t invsbox[256] =
	{
	
	};



uint8_t AES_T::gf256mul(uint8_t a, uint8_t b, uint8_t reducer)
	{
	uint8_t i;
	uint8_t p=0,t;
	for(i=0; i<8; ++i)
		{
		if(b&1)
			p ^= a;
		t=a&0x80;
		a<<=1;
		if(t)
			a ^= reducer;
		b>>=1;
		}
	return p;
	}

void AES_T::rotword(void* a)
	{
	uint8_t t;
	t=((uint8_t*)a)[0];
	((uint8_t*)a)[0] = ((uint8_t*)a)[1];
	((uint8_t*)a)[1] = ((uint8_t*)a)[2];
	((uint8_t*)a)[2] = ((uint8_t*)a)[3];
	((uint8_t*)a)[3] = t;
	}

const uint8_t rc_tab[] = { 0x01, 0x02, 0x04, 0x08,
						   0x10, 0x20, 0x40, 0x80,
						   0x1b, 0x36
						 };

void AES_T::init(void* key, uint16_t keysize_b, aes128_ctx_t* ctx)
	{
	uint8_t hi,i,nk, next_nk;
	uint8_t rc=0;
	union
		{
		uint32_t v32;
		uint8_t  v8[4];
		} tmp;
	nk=keysize_b>>5; /* 4, 6, 8 */
	hi=4*(nk+6+1);
	memcpy(ctx, key, keysize_b/8);
	next_nk = nk;
	for(i=nk; i<hi; ++i)
		{
		tmp.v32 = ((uint32_t*)(ctx->key[0].ks))[i-1];
		if(i!=next_nk)
			{
			if(nk==8 && i%8==4)
				{
				tmp.v8[0] = sbox[(tmp.v8[0])];
				tmp.v8[1] = sbox[(tmp.v8[1])];
				tmp.v8[2] = sbox[(tmp.v8[2])];
				tmp.v8[3] = sbox[(tmp.v8[3])];
				}
			}
		else
			{
			next_nk += nk;
			rotword(&(tmp.v32));
			tmp.v8[0] = sbox[(tmp.v8[0])];
			tmp.v8[1] = sbox[(tmp.v8[1])];
			tmp.v8[2] = sbox[(tmp.v8[2])];
			tmp.v8[3] = sbox[(tmp.v8[3])];
			tmp.v8[0] ^= rc_tab[rc];
			rc++;
			}
		((uint32_t*)(ctx->key[0].ks))[i] = ((uint32_t*)(ctx->key[0].ks))[i-nk]
										   ^ tmp.v32;
		}
	}


void AES_T::shiftcol(void* data, uint8_t shift)
	{
	uint8_t tmp[4];
	tmp[0] = ((uint8_t*)data)[ 0];
	tmp[1] = ((uint8_t*)data)[ 4];
	tmp[2] = ((uint8_t*)data)[ 8];
	tmp[3] = ((uint8_t*)data)[12];
	((uint8_t*)data)[ 0] = tmp[(shift+0)&3];
	((uint8_t*)data)[ 4] = tmp[(shift+1)&3];
	((uint8_t*)data)[ 8] = tmp[(shift+2)&3];
	((uint8_t*)data)[12] = tmp[(shift+3)&3];
	}

#define GF256MUL_1(a) (a)
#define GF256MUL_2(a) (gf256mul(2, (a), 0x1b))
#define GF256MUL_3(a) (gf256mul(3, (a), 0x1b))

void AES_T::enc_round(AES_T::cipher_state_t* state, const AES_T::roundkey_t* k)
	{
	uint8_t tmp[16], t;
	uint8_t i;
	/* subBytes */
	for(i=0; i<16; ++i)
		{
		tmp[i] = sbox[state->s[i]];
		}
	/* shiftRows */
	shiftcol(tmp+1, 1);
	shiftcol(tmp+2, 2);
	shiftcol(tmp+3, 3);
	/* mixColums */
	for(i=0; i<4; ++i)
		{
		t = tmp[4*i+0] ^ tmp[4*i+1] ^ tmp[4*i+2] ^ tmp[4*i+3];
		state->s[4*i+0] =
			GF256MUL_2(tmp[4*i+0]^tmp[4*i+1])
			^ tmp[4*i+0]
			^ t;
		state->s[4*i+1] =
			GF256MUL_2(tmp[4*i+1]^tmp[4*i+2])
			^ tmp[4*i+1]
			^ t;
		state->s[4*i+2] =
			GF256MUL_2(tmp[4*i+2]^tmp[4*i+3])
			^ tmp[4*i+2]
			^ t;
		state->s[4*i+3] =
			GF256MUL_2(tmp[4*i+3]^tmp[4*i+0])
			^ tmp[4*i+3]
			^ t;
		}

	/* addKey */
	for(i=0; i<16; ++i)
		{
		state->s[i] ^= k->ks[i];
		}
	}


void AES_T::enc_lastround(AES_T::cipher_state_t* state,const AES_T::roundkey_t* k)
	{
	uint8_t i;
	/* subBytes */
	for(i=0; i<16; ++i)
		{
		state->s[i] = sbox[state->s[i]];
		}
	/* shiftRows */
	shiftcol(state->s+1, 1);
	shiftcol(state->s+2, 2);
	shiftcol(state->s+3, 3);
	/* keyAdd */
	for(i=0; i<16; ++i)
		{
		state->s[i] ^= k->ks[i];
		}
	}

void AES_T::encrypt_core(AES_T::cipher_state_t* state, const AES_T::aes128_ctx_t* ks, uint8_t rounds)
	{
	uint8_t i;
	for(i=0; i<16; ++i)
		{
		state->s[i] ^= ks->key[0].ks[i];
		}
	i=1;
	for(; rounds>1; --rounds)
		{
		enc_round(state, &(ks->key[i]));
		++i;
		}
	enc_lastround(state, &(ks->key[i]));
	}


void AES_T::invshiftrow(void* data, uint8_t shift)
	{
	uint8_t tmp[4];
	tmp[0] = ((uint8_t*)data)[(4+0-shift)&3];
	tmp[1] = ((uint8_t*)data)[(4+1-shift)&3];
	tmp[2] = ((uint8_t*)data)[(4+2-shift)&3];
	tmp[3] = ((uint8_t*)data)[(4+3-shift)&3];
	memcpy(data, tmp, 4);
	}

void AES_T::invshiftcol(void* data, uint8_t shift)
	{
	uint8_t tmp[4];
	tmp[0] = ((uint8_t*)data)[ 0];
	tmp[1] = ((uint8_t*)data)[ 4];
	tmp[2] = ((uint8_t*)data)[ 8];
	tmp[3] = ((uint8_t*)data)[12];
	((uint8_t*)data)[ 0] = tmp[(4-shift+0)&3];
	((uint8_t*)data)[ 4] = tmp[(4-shift+1)&3];
	((uint8_t*)data)[ 8] = tmp[(4-shift+2)&3];
	((uint8_t*)data)[12] = tmp[(4-shift+3)&3];
	}

void AES_T::dec_round(AES_T::cipher_state_t* state, const AES_T::roundkey_t* k)
	{
	uint8_t tmp[16];
	uint8_t i;
	uint8_t t,u,v,w;
	/* keyAdd */
	for(i=0; i<16; ++i)
		{
		tmp[i] = state->s[i] ^ k->ks[i];
		}
	/* mixColums */
	for(i=0; i<4; ++i)
		{
		t = tmp[4*i+3] ^ tmp[4*i+2];
		u = tmp[4*i+1] ^ tmp[4*i+0];
		v = t ^ u;
		v = gf256mul(0x09, v, 0x1b);
		w = v ^ gf256mul(0x04, tmp[4*i+2] ^ tmp[4*i+0], 0x1b);
		v = v ^ gf256mul(0x04, tmp[4*i+3] ^ tmp[4*i+1], 0x1b);
		state->s[4*i+3] = tmp[4*i+3] ^ v ^ gf256mul(0x02, tmp[4*i+0] ^ tmp[4*i+3], 0x1b);
		state->s[4*i+2] = tmp[4*i+2] ^ w ^ gf256mul(0x02, t, 0x1b);
		state->s[4*i+1] = tmp[4*i+1] ^ v ^ gf256mul(0x02, tmp[4*i+2] ^ tmp[4*i+1], 0x1b);
		state->s[4*i+0] = tmp[4*i+0] ^ w ^ gf256mul(0x02, u, 0x1b);

		/*
		state->s[4*i+0] =
			  gf256mul(0xe, tmp[4*i+0], 0x1b)
			^ gf256mul(0xb, tmp[4*i+1], 0x1b)
			^ gf256mul(0xd, tmp[4*i+2], 0x1b)
			^ gf256mul(0x9, tmp[4*i+3], 0x1b);
		state->s[4*i+1] =
			  gf256mul(0x9, tmp[4*i+0], 0x1b)
			^ gf256mul(0xe, tmp[4*i+1], 0x1b)
			^ gf256mul(0xb, tmp[4*i+2], 0x1b)
			^ gf256mul(0xd, tmp[4*i+3], 0x1b);
		state->s[4*i+2] =
			  gf256mul(0xd, tmp[4*i+0], 0x1b)
			^ gf256mul(0x9, tmp[4*i+1], 0x1b)
			^ gf256mul(0xe, tmp[4*i+2], 0x1b)
			^ gf256mul(0xb, tmp[4*i+3], 0x1b);
		state->s[4*i+3] =
			  gf256mul(0xb, tmp[4*i+0], 0x1b)
			^ gf256mul(0xd, tmp[4*i+1], 0x1b)
			^ gf256mul(0x9, tmp[4*i+2], 0x1b)
			^ gf256mul(0xe, tmp[4*i+3], 0x1b);
		*/
		}
	/* shiftRows */
	invshiftcol(state->s+1, 1);
	invshiftcol(state->s+2, 2);
	invshiftcol(state->s+3, 3);
	/* subBytes */
	for(i=0; i<16; ++i)
		{
		state->s[i] = invsbox[state->s[i]];
		}
	}


void AES_T::dec_firstround(AES_T::cipher_state_t* state, const AES_T::roundkey_t* k)
	{
	uint8_t i;
	/* keyAdd */
	for(i=0; i<16; ++i)
		{
		state->s[i] ^= k->ks[i];
		}
	/* shiftRows */
	invshiftcol(state->s+1, 1);
	invshiftcol(state->s+2, 2);
	invshiftcol(state->s+3, 3);
	/* subBytes */
	for(i=0; i<16; ++i)
		{
		state->s[i] = invsbox[state->s[i]];
		}
	}

void AES_T::decrypt_core(AES_T::cipher_state_t* state, const AES_T::aes128_ctx_t* ks, uint8_t rounds)
	{
	uint8_t i;
	dec_firstround(state, &(ks->key[i=rounds]));
	for(; rounds>1; --rounds)
		{
		--i;
		dec_round(state, &(ks->key[i]));
		}
	for(i=0; i<16; ++i)
		{
		state->s[i] ^= ks->key[0].ks[i];
		}
	}

void AES_T::aes128_dec(uint8_t* key, uint8_t* data, AES_T::mode_t mode, uint8_t* iv)
	{
	aes128_ctx_t ctx = {};
	init(key, 128, (aes128_ctx_t*)&ctx);
	switch(mode)
		{
		case ECB:
			{
			decrypt_core((cipher_state_t*)data, (aes128_ctx_t*)&ctx, 10);
			return;	
			};
		case CBC:
			{
			if(!iv)
				{
				iv = iv_dec;
				}
			uint16_t tmp_data[16] = {};
			memcpy(tmp_data,data,16);
			decrypt_core((cipher_state_t*)data, (aes128_ctx_t*)&ctx, 10);
			_xor(data,iv);
			memcpy(iv,tmp_data,16);
			return;
			}
		default: return;
		}
	}


void AES_T::aes128_enc(uint8_t* key,uint8_t* data, AES_T::mode_t mode, uint8_t* iv)
	{
	aes128_ctx_t ctx = {};
	init(key, 128, (aes128_ctx_t*)&ctx);		
	switch(mode)
		{
		case ECB:
			{
			encrypt_core((cipher_state_t*)data, (aes128_ctx_t*)&ctx, 10);
			return;	
			};
		case CBC:
			{
			if(!iv)
				{
				iv = iv_enc;
				}
			_xor(data,iv);
			encrypt_core((cipher_state_t*)data, (aes128_ctx_t*)&ctx, 10);
			memcpy(iv,data,16);
			return;
			}
		default: return;
		}
	}


void AES_T::_xor(uint8_t* a, uint8_t* b, uint8_t a_len, uint8_t b_len)
	{
	uint8_t len = b_len;
	if(a <= b)
		{
		len = a_len;
		}
	for(uint8_t i =0; i<len; i++)
		{
		a[i] ^= b[i];
		}
	}
